...
function DS_goDirections( from, to ) {
  ...
  DS_directions.load('from: ' + from + ' to: ' + to,
      {getSteps: true, getPolyline: true});
}

function DS_goDirections2( oLat, oLong, dLat, dLong ) {
  ...
  from = '' + oLat + ',' + oLong + '';
  to = '' + dLat + ',' + dLong + '';
  wayPoints = new Array( from, to );
  
  google.maps.Event.addListener( DS_directions, 'load', DS_directionsLoaded );

  
  DS_directions.loadFromWaypoints( wayPoints, 
    { getSteps: true, getPolyline: true } );
}
...
function DS_flyToStep2(stepNum) {
  var step = DS_steps[stepNum];
...
function DS_flyToLatLng2( loc, heading, zoom ) {
  var la = DS_ge.createLookAt('');
  la.set(loc.lat(), loc.lng(),
      10, // altitude
      DS_ge.ALTITUDE_RELATIVE_TO_GROUND,
